...
int CreateDelaunayTriangulation(sf::VertexArray* verts, int n_verts, 
    ...
    for (i = 0; i < n_verts; ++i) {
        //        if (verts[i].IsValid()) {
        sf::Vector2f vertex=transform.transformPoint((*verts)[i].position);
        p[count].x = vertex.x;
        p[count].y = vertex.y;
        ++count;
        //        }
    ...
    for (int i = 0; i < triangleCount; i++) {
        ...
        p0 = sf::Vector2f(p[v[i].p1].x, p[v[i].p1].y);
        p1 = sf::Vector2f(p[v[i].p2].x, p[v[i].p2].y);
        p2 = sf::Vector2f(p[v[i].p3].x, p[v[i].p3].y);
