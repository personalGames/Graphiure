/* 
 * File:   TinyXmlTest.cpp
 * Author: dorian
 *
 * Created on 12-feb-2015, 16:29:39
 */
...
{
	XMLDocument doc;
	doc.LoadFile( "etc/resources/dream.xml" );
...
int main( int argc, const char ** argv )
{
		...
		_CrtSetDbgFlag(_CrtSetDbgFlag(_CRTDBG_REPORT_FLAG) | _CRTDBG_LEAK_CHECK_DF);
			...
			mkdir( "resources/out/" );
                ...
                #endif
	#else
		mkdir( "etc/resources/out/", S_IRWXU | S_IRWXG | S_IROTH | S_IXOTH);
	#endif
	...
	FILE* fp = fopen( "etc/resources/dream.xml", "r" );
	if ( !fp ) {
	...
	}
	{
		...
		doc->SaveFile( "./etc/resources/out/pretty.xml" );
		doc->SaveFile( "./etc/resources/out/compact.xml", true );
		delete doc;
	}
	{
		// Test: Dream
		...
		doc.SaveFile( "etc/resources/out/dreamout.xml" );
		doc.PrintError();
		...
		XMLDocument doc2;
		doc2.LoadFile( "etc/resources/out/dreamout.xml" );
	...
	{
		XMLDocument doc;
		doc.LoadFile( "etc/resources/utf8test.xml" );
		...
		doc.SaveFile( "etc/resources/out/utf8testout.xml" );

		// Check the round trip.
		int okay = 0;

		FILE* saved  = fopen( "etc/resources/out/utf8testout.xml", "r" );
		FILE* verify = fopen( "etc/resources/utf8testverify.xml", "r" );
...

	{
		...
		FILE* textfile = fopen( "etc/resources/out/textfile.txt", "w" );
		if ( textfile )
		{
        ...
        textfile = fopen( "etc/resources/out/textfile.txt", "r" );
...

	{
		...
		XMLDocument doc;
		doc.Parse( doctype );
		doc.SaveFile( "etc/resources/out/test7.xml" );
		doc.DeleteChild( doc.RootElement() );
		doc.LoadFile( "etc/resources/out/test7.xml" );
		doc.Print();
	...
	{
		XMLDocument doc;
		XMLError error = doc.LoadFile( "etc/resources/empty.xml" );
...

	{
        ...
        static const char* xml_bom_preservation  = "\xef\xbb\xbf<element/>\n";
        {
			...
			doc.SaveFile( "etc/resources/bomtest.xml" );
        }
		{
			XMLDocument doc;
			doc.LoadFile( "etc/resources/bomtest.xml" );
